...
def residual_unit_int8(data, channel, num_filter, stride, dim_match, name, bottle_neck=True,
    ...
    if bottle_neck:
        ...
        weight_conv1 = mx.sym.Variable(name=name + "_conv1_weight",
                                       shape=(num_filter // 4, channel, 1, 1), init=init)
        ...
        weight_conv2 = mx.sym.Variable(name=name + "_conv2_weight",
                                       shape=(num_filter // 4, num_filter // 4, 3, 3), init=init)
        ...
        weight_conv3 = mx.sym.Variable(name=name + "_conv3_weight",
                                       shape=(num_filter, num_filter // 4, 1, 1), init=init)
        ...
        if dim_match:
            ...
            weight_sc = mx.sym.Variable(name=name + "_sc_weight",
                                        shape=(num_filter, channel, 1, 1), init=init)
        ...
        # weight to be quantized
        weight_conv1 = mx.sym.Variable(name=name + "_conv1_weight",
                                       shape=(num_filter, channel, 3, 3), init=init)
        ...
        relu2_q = mx.sym.Quantization_int8(data=act2, is_weight=False,
                                           ema_decay=0.99, delay_quant=0, is_train=is_train, quant_mod=quant_mod)
        weight_conv2 = mx.sym.Variable(name=name + "_conv2_weight",
                                       shape=(num_filter, num_filter, 3, 3), init=init)
...
def resnet_int8(units, num_stage, filter_list, num_class, data_type, bottle_neck=True,
    ...
    weight = mx.sym.Variable(name="conv0_weight",
                             shape=(filter_list[0], 3, 7, 7), init=init)
